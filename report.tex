...
\begin{titlepage}
  ...
  \begin{minipage}{0.75\textwidth}
    ...
    \begin{tabular}{rl}
      ...
      \textbf{GitHub Repo} & \href{https://github.com/sidnovates/NoSQL-Hadoop-Assignment}{NoSQL-Hadoop-Assignment} \\[4pt]
...
\begin{algorithm}
\caption{Top-50 Word Frequency --- Method 1}
\begin{algorithmic}[1]

\State \textbf{class Mapper}

\State \textbf{method} Setup()
\State \quad Load skip patterns from Distributed Cache into set $S$

\State \textbf{method} Map(key, line)
\State \quad cleanLine $\gets$ lowercase(line)
\State \quad tokens $\gets$ split(cleanLine, \verb|[^a-z]+|)
\State \quad \textbf{for each} token $t$ in tokens \textbf{do}
\State \qquad \textbf{if} isValidToken($t$, $S$) \textbf{then}
\State \qquad\qquad Emit($t$, 1)
...
\State \textbf{method} Reduce(word, counts)
\State \quad $s \gets \sum counts$
\State \quad Emit(word, $s$)
...
\begin{algorithm}
\caption{Top-50 Word Frequency --- Method 2 (In-Reducer Sorting)}
\begin{algorithmic}[1]
...
\State \quad Initialize empty map $M$

\State \textbf{method} Reduce(word, counts)
\State \quad $M[\text{word}] \gets \sum counts$

\State \textbf{method} Cleanup()
\State \quad $L \gets$ sort $M$ in descending order of frequency
\State \quad \textbf{for each} entry $(w, c)$ in first 50 elements of $L$ \textbf{do}
\State \qquad Emit($w$, $c$)
\State \quad \textbf{end for}
...
\begin{tcolorbox}[title=Post-processing command for Method 1, colback=black!5!white, colframe=black]
\texttt{hdfs dfs -cat /Problem1/1a\_ExtSort/part-r-$*$ | sort -k2 -nr | head -50 > top50.txt}
\end{tcolorbox}
...
The following layout presents the final output for the top-50 most frequent non-stop words alongside the cloud environment screenshot.

\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.58\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Problem1/1a/image.png}
    \caption{Top-50 words output from the external sorting phase.}
    \label{fig:top50_output}
  \end{minipage}\hfill
  \begin{minipage}[b]{0.38\textwidth}
\begin{lstlisting}[style=bashcode, basicstyle=\ttfamily\tiny, numbers=none, breaklines=false, title={Terminal Output}]
apos    273057
quot    268016
http    79117
category 60536
...
\end{lstlisting}
  \end{minipage}
\end{figure}
\subsubsection{YARN ResourceManager Web UI }
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{ScreenShots/Problem1/1a/YARN1.png}
    \caption{ACCEPTED}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{ScreenShots/Problem1/1a/YARN2.png}
    \caption{RUNNING}
  \end{subfigure}

  \caption{YARN Resource Allocation UI}
\end{figure}

\subsubsection{Run Time Terminal Output }
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{ScreenShots/Problem1/1a/Runtime.png}
    \caption{In-Reducer Method}
  \end{subfigure}\hfill
\begin{subfigure}{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth, trim=0 0 0 150, clip]{ScreenShots/Problem1/1a/Runtime2.png}
  \caption{Shell Sort}
\end{subfigure}
...
\begin{algorithm}
\caption{Co-occurring Word Matrix --- Pairs Approach}
\begin{algorithmic}[1]

\State \textbf{// Setup: load top-50 words and distance d}
\State \textbf{function} setup(context):
\State \quad top50 $\gets$ load\_top\_words\_from\_cache()
\State \quad d $\gets$ get\_config("distance")

\vspace{0.5em}
\State \textbf{// Mapper}
\State \textbf{function} Map(key, line):
\State \quad tokens $\gets$ line.toLowerCase().split()
\State \quad \textbf{for} $i = 0$ to tokens.length $- 1$:
\State \qquad $w1 \gets$ tokens[i]
\State \qquad \textbf{if} $w1$ in top50:
\State \qquad\quad \textbf{// Look ahead up to distance d}
\State \qquad\quad \textbf{for} $j = i + 1$ to $\min(i + d,\; \text{tokens.length} - 1)$:
\State \qquad\qquad $w2 \gets$ tokens[j]
\State \qquad\qquad \textbf{if} $w2$ in top50:
\State \qquad\qquad\quad emit(( $w1$, $w2$ ), 1)

\vspace{0.5em}
\State \textbf{// Reducer}
\State \textbf{function} Reduce(pair, counts):
\State \quad Emit(pair, sum(counts))
...
\subsubsection{Output Cases -- Pairs Approach}

\begin{figure}[H]
  \centering
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1b/Pairs_D1.png}
    \caption{Pairs approach, $d = 1$}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1b/Pairs_D2.png}
    \caption{Pairs approach, $d = 2$}
  \end{minipage}
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 300 300 0, clip]{ScreenShots/Problem1/1b/Pairs_D3.png}
    \caption{Pairs approach, $d = 3$}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1b/Pairs_D4.png}
    \caption{Pairs approach, $d = 4$}
  \end{minipage}
\end{figure}
...
\begin{algorithm}
\caption{Co-occurring Word Matrix --- Stripes Approach}
\begin{algorithmic}[1]
...
\State \qquad $w1 \gets$ tokens[i]
\State \qquad \textbf{if} $w1$ in top50:
\State \qquad\quad stripe $\gets$ \{\} \Comment{Associative array}
\State \qquad\quad \textbf{for} $j = i + 1$ to $\min(i + d,\; \text{tokens.length} - 1)$:
\State \qquad\qquad $w2 \gets$ tokens[j]
\State \qquad\qquad \textbf{if} $w2$ in top50:
\State \qquad\qquad\quad stripe[$w2$] += 1
\State \qquad\quad \textbf{end for}
\State \qquad\quad \textbf{if} stripe is not empty:
\State \qquad\qquad emit($w1$, stripe)
\State \quad \textbf{end for}

\vspace{0.5em}
\State \textbf{// Reducer}
\State \textbf{function} Reduce(word, stripes):
\State \quad merged\_stripe $\gets$ \{\}
\State \quad \textbf{for each} stripe in stripes:
\State \qquad \textbf{for each} (neighbor, count) in stripe:
\State \qquad\quad merged\_stripe[neighbor] += count
\State \quad Emit(word, merged\_stripe)
...
\end{infobox}
\subsubsection{Output Cases -- Stripes Approach}

\begin{figure}[H]
  \centering
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1c/Stripes_D1.png}
    \caption{Stripes approach, $d = 1$}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth,  trim=0 0 0 0, clip]{ScreenShots/Problem1/1c/Stripes_D2.png}
    \caption{Stripes approach, $d = 2$}
  \end{minipage}
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth ,trim=0 0 0 0, clip]{ScreenShots/Problem1/1c/Stripes_D3.png}
    \caption{Stripes approach, $d = 3$}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth,  trim=0 0 0 0, clip]{ScreenShots/Problem1/1c/Stripes_D4.png}
    \caption{Stripes approach, $d = 4$}
  \end{minipage}
\end{figure}
...
\begin{algorithm}
\caption{Map-Function Level Local Aggregation}
\begin{algorithmic}[1]

\State \textbf{function} Map(key, line):
\State \quad local\_counts $\gets$ \{\} \Comment{Initialized inside function}
\State \quad words $\gets$ tokenize(line)
\State \quad \textbf{for} $i = 0$ to words.length $- 1$:
\State \qquad \textbf{for} $j = i + 1$ to $\min(i + d,\; \text{words.length} - 1)$:
\State \qquad\quad local\_counts[(words[i], words[j])] += 1

\vspace{0.5em}
\State \quad \textbf{// Emit results for THIS map call}
\State \quad \textbf{for each} (pair, count) in local\_counts:
\State \qquad emit(pair, count)
...
\begin{algorithm}
\caption{Map-Class Level Local Aggregation}
\begin{algorithmic}[1]

\State \textbf{// State maintained across map() calls}
\State globalMap $\gets$ \{\} \Comment{Class instance variable}

\State \textbf{function} setup(context):
\State \quad globalMap $\gets$ \{\} \Comment{Initialized once per task}

\State \textbf{function} Map(key, line):
\State \quad words $\gets$ tokenize(line)
\State \quad \textbf{for} $i = 0$ to words.length $- 1$:
\State \qquad \textbf{for} $j = i + 1$ to $\min(i + d,\; \text{words.length} - 1)$:
\State \qquad\quad globalMap[(words[i], words[j])] += 1

\State \textbf{function} cleanup(context):
\State \quad \textbf{// Emit cumulative results for the ENTIRE task}
\State \quad \textbf{for each} (pair, count) in globalMap:
\State \qquad emit(pair, count)
...
\begin{algorithm}
\caption{Map-Function Level Local Aggregation --- Stripes}
\begin{algorithmic}[1]

\State \textbf{function} Map(key, line):
\State \quad localStripes $\gets$ \{\} \Comment{Map<Word, Stripe> (per record)}
\State \quad words $\gets$ tokenize(line)
\State \quad \textbf{for} $i = 0$ to words.length $- 1$:
\State \qquad \textbf{if} words[i] in top50:
\State \qquad\quad \textbf{for} $j = i + 1$ to $\min(i + d,\; \text{words.length} - 1)$:
\State \qquad\qquad \textbf{if} words[j] in top50:
\State \qquad\qquad\quad localStripes[words[i]][words[j]] += 1

\vspace{0.5em}
\State \quad \textbf{// Emit results for THIS record}
\State \quad \textbf{for each} (word, stripe) in localStripes:
\State \qquad emit(word, stripe)

\end{algorithmic}
\end{algorithm}

\begin{algorithm}
\caption{Map-Class Level Local Aggregation --- Stripes (In-Mapper Combining)}
\begin{algorithmic}[1]

\State \textbf{// State maintained across map() calls}
\State globalStripes $\gets$ \{\} \Comment{Map<Word, Stripe>}

\State \textbf{function} setup(context):
\State \quad globalStripes $\gets$ \{\}
...
\State \qquad\quad \textbf{for} $j = i + 1$ to $\min(i + d,\; \text{words.length} - 1)$:
\State \qquad\qquad \textbf{if} words[j] in top50:
\State \qquad\qquad\quad globalStripes[words[i]][words[j]] += 1

\State \textbf{function} cleanup(context):
\State \quad \textbf{// Emit cumulative stripes for the ENTIRE task}
\State \quad \textbf{for each} (word, stripe) in globalStripes:
\State \qquad emit(word, stripe)
...
\subsubsection{Output Cases -- Local Aggregation}

\begin{infobox}
  \textbf{Representative Results:} \\
  We present results for local aggregation across both Pairs and Stripes approaches for window sizes $d \in \{1,2,3,4\}$. 
  For each approach, both Map-Class-Level Aggregation and Map-Function-Level Aggregation are shown. 
\end{infobox}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapClassLevelAgg/D1.png}
    \caption{$d = 1$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapClassLevelAgg/D2.png}
    \caption{$d = 2$}
  \end{subfigure}
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapClassLevelAgg/D3.png}
    \caption{$d = 3$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapClassLevelAgg/D4.png}
    \caption{$d = 4$}
  \end{subfigure}
  \caption{Stripes (Map-Class Aggregation) for different window sizes}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapClassLevelAgg/D1.png}
    \caption{$d = 1$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapClassLevelAgg/D2.png}
    \caption{$d = 2$}
  \end{subfigure}
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapClassLevelAgg/D3.png}
    \caption{$d = 3$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapClassLevelAgg/D4.png}
    \caption{$d = 4$}
  \end{subfigure}

  \caption{Pairs (Map-Class Aggregation) for different window sizes}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapFunctionLevelAgg/D1.png}
    \caption{$d = 1$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapFunctionLevelAgg/D2.png}
    \caption{$d = 2$}
  \end{subfigure}
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 0 0, clip]{ScreenShots/Problem1/1d/Stripes/MapFunctionLevelAgg/D3.png}
    \caption{$d = 3$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    ...
    \caption{$d = 4$}
  \end{subfigure}
  \caption{Stripes (Map-Function Aggregation) for different window sizes}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapFunctionLevelAgg/D1.png}
    \caption{$d = 1$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapFunctionLevelAgg/D3.png}
    \caption{$d = 2$}
  \end{subfigure}
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth, trim=0 0 300 0, clip]{ScreenShots/Problem1/1d/Pairs/MapFunctionLevelAgg/D4.png}
    \caption{$d = 4$}
  \end{subfigure}

  \caption{Pairs (Map-Functions Aggregation) for different window sizes}
\end{figure}
\clearpage
...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[width=0.8\textwidth]{pairs_local_agg.png}}
    \caption{Pairs Approach: Map-Function vs.\ Map-Class}

    \vspace{0.5cm}
    
    \fbox{\includegraphics[width=0.8\textwidth]{stripes_local_agg.png}}
...
\State \textbf{// Setup: load stopwords and initialize stemmer}
\State \textbf{function} setup(context):
\State \quad stopwords $\gets$ load\_from\_cache()
\State \quad stemmer $\gets$ new PorterStemmer()

\vspace{0.5em}
\State \textbf{// Mapper}
\State \textbf{function} Map(filePath, line):
\State \quad docId $\gets$ extract\_filename\_without\_extension(filePath)
\State \quad tokens $\gets$ tokenize(line.toLowerCase())
\State \quad seenThisLine $\gets$ \{\}

\State \quad \textbf{for each} token in tokens:
\State \qquad \textbf{if} not isValidToken(token):
\State \qquad\quad continue
\State \qquad \textbf{if} token in stopwords:
\State \qquad\quad continue
\State \qquad stemmed $\gets$ stemmer.stem(token)
\State \qquad \textbf{if} not isValidToken(stemmed):
\State \qquad\quad continue
\State \qquad \textbf{if} stemmed in stopwords:
\State \qquad\quad continue
\State \qquad \textbf{if} stemmed not in seenThisLine:
\State \qquad\quad seenThisLine.add(stemmed)
\State \qquad\quad emit(stemmed, docId)

\vspace{0.5em}
\State \textbf{// Reducer}
\State \textbf{function} Reduce(term, docIds):
\State \quad distinctDocs $\gets$ \{\}
\State \quad \textbf{for each} docId in docIds:
\State \qquad distinctDocs.add(docId)
\State \quad emit(term, size(distinctDocs))
...
\State \textbf{// Setup: load DF map from Distributed Cache (Mapper \& Reducer)}
\State \textbf{function} setup(context):
\State \quad df\_map $\gets$ load\_tsv\_as\_map(cache\_file) \Comment{Maps term $\to$ DF}

\vspace{0.5em}
\State \textbf{// Mapper (Stripes)}
\State \textbf{function} Map(filePath, line):
\State \quad docID $\gets$ extract\_filename\_without\_extension(filePath)
\State \quad stripe $\gets$ docStripes[docID] if exists else \{\}
\State \quad tokens $\gets$ tokenize(line.toLowerCase())
\State \quad \textbf{for each} token in tokens:
\State \qquad stemmed $\gets$ stem(token)
\State \qquad \textbf{if} stemmed in df\_map:
\State \qquad\quad stripe[stemmed] += 1
\State \quad docStripes[docID] $\gets$ stripe

\State \textbf{function} cleanup(context):
\State \quad \textbf{for each} (docID, stripe) in docStripes:
\State \qquad stripeString $\gets$ serialize\_stripe(stripe) \Comment{term:count|term:count|...}
\State \qquad emit(docID, stripeString)

\vspace{0.5em}
\State \textbf{// Reducer}
\State \textbf{function} Reduce(docID, stripeStrings):
\State \quad termCounts $\gets$ \{\}
\State \quad \textbf{for each} stripeString in stripeStrings:
\State \qquad pairs $\gets$ split(stripeString, "|")
\State \qquad \textbf{for each} pair in pairs:
\State \qquad\quad (term, count) $\gets$ split(pair, ":")
\State \qquad\quad termCounts[term] += parseInt(count)

\State \quad \textbf{for each} (term, tf) in termCounts:
\State \qquad df $\gets$ df\_map[term]
\State \qquad score $\gets$ tf * log(10000.0 / df + 1)
\State \qquad emit(docID, term + "\textbackslash t" + format(score, 6))
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{ScreenShots/Problem2/TIFIDF_Scores_Long.png}
